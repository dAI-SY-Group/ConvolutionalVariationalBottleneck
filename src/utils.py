...
def get_available_datasets():
    return {
        ...
        'MedMNISTDerma': 'MedMNISTDerma.yaml',
        'MedMNISTBlood': 'MedMNISTBlood.yaml',
        'MedMNISTRetina': 'MedMNISTRetina.yaml',
        'MedMNISTPneumonia': 'MedMNISTPneumonia.yaml',        
    }
...
def get_dataset_mean_dev_tensors(dataset):
    ...
    assert dataset in available_datasets, f'Dataset {dataset} is not available. Try one of {list(available_datasets.keys())}.'
    config = build_config(os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'configs', 'data', available_datasets[dataset]))
